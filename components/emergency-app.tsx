...
import type { ActivityLog, EventRecord, OperationalRecord, ProductModule, Profile, RiskRecord, Role, TaskRecord } from "@/lib/types";
import { AdminPage, CityPage, CommandPage, DataPage, DutyPage, InventoryPage, LogsPage, OverviewPage, PlansPage, PortalPage, ResourcesPage, RisksPage, TyphoonPage } from "./product-pages";
...
export function EmergencyApp() {
  ...
  const load = useCallback(async () => {
    ...
    setUser(current);
    if (!current) { setData(loadLocal()); setRole("member"); setLoading(false); return; }
    const profileResult = await supabase.from("profiles").select("id,email,display_name,role,organization,job_title,created_at").eq("id", current.id).maybeSingle();
    ...
    setRole(currentRole);
    const [events, tasks, risks, logs, records] = await Promise.all([
    ...
    const failure = [events, tasks, risks, logs, records].find((result) => result.error)?.error;
    ...
    if (currentRole === "admin") {
      const members = await supabase.from("profiles").select("id,email,display_name,role,organization,job_title,created_at").order("created_at", { ascending: false });
  ...
  function requireWrite() { if (!canWrite(role)) { setNotice("当前角色为只读角色。"); return false; } return true; }
  ...
  async function addTask(form: FormData, eventId?: string, planId?: string) {
    if (!requireWrite()) return;
    const record: TaskRecord = { id: uid(), event_id: eventId || null, plan_id: planId || null, title: String(form.get("title")), assignee: String(form.get("assignee")), resource: String(form.get("resource") || ""), channel: String(form.get("channel") || "平台内"), status: "待查阅", due_minutes: Number(form.get("due") || 30), created_at: now() };
  ...
  async function addRisk(form: FormData) {
    if (!requireWrite()) return;
    const record: RiskRecord = { id: uid(), name: String(form.get("name")), area: String(form.get("area")), record_type: String(form.get("recordType")), source: String(form.get("source")), change_type: String(form.get("changeType")) as RiskRecord["change_type"], old_value: String(form.get("oldValue") || ""), new_value: String(form.get("newValue")), assigned_org: String(form.get("assignedOrg") || ""), status: "待派单", created_at: now() };
  ...
  const common = { records: data.records, writable: canWrite(role), onAdd: addRecord, onUpdate: updateRecord, onDelete: removeRecord };
  return <div className="shell"><aside><div className="brand"><span>湖</span><div><b>西湖应急</b><small>INTEGRATED OPERATIONS</small></div></div><nav>{productPages.filter((item) => item.key !== "admin" || role === "admin").map((item, index) => <button key={item.key} className={page === item.key ? "active" : ""} onClick={() => setPage(item.key)} title={item.group}><i>{String(index + 1).padStart(2, "0")}</i>{item.label}</button>)}</nav><div className="aside-foot"><b>{user ? "SUPABASE 云端" : "本机持久化"}</b><small>{user?.email || "访客模式"}</small></div></aside><main><header><span>西湖区应急管理综合平台 / {productPages.find((item) => item.key === page)?.label}</span><div className="actions"><span className={user ? "badge" : "badge orange"}>{user ? `云端已连接 · ${role}` : "本地产品体验"}</span>{user ? <button onClick={signOut}>退出</button> : <button onClick={() => setShowAuth(!showAuth)}>登录</button>}</div></header><div className="workspace">{showAuth && !user && <AuthPanel onDone={() => { setShowAuth(false); void load(); }} />}{notice && <div className="notice"><span>{notice}</span><button onClick={() => setNotice("")}>关闭</button></div>}
    {page === "overview" && <OverviewPage stats={stats} data={data} role={role} onSeed={seed} onExport={exportData} onImport={() => importRef.current?.click()} importRef={importRef} importData={importData} />}
    {page === "portal" && <PortalPage data={data} role={role} />}{page === "typhoon" && <TyphoonPage {...common} onCreateEvent={createEventFromAlert} />}
    {page === "plans" && <PlansPage records={data.records} events={data.events} writable={canWrite(role)} onAdd={addRecord} onDelete={removeRecord} onStart={startPlan} />}
    {page === "command" && <CommandPage events={data.events} tasks={data.tasks} writable={canWrite(role)} onAddEvent={addEvent} onProgressEvent={progressEvent} onDeleteEvent={removeEvent} onAddTask={addTask} onProgressTask={progressTask} onDeleteTask={removeTask} />}
    {page === "resources" && <ResourcesPage {...common} />}{page === "inventory" && <InventoryPage {...common} />}
    {page === "risks" && <RisksPage risks={data.risks} writable={canWrite(role)} onAdd={addRisk} onProgress={progressRisk} onDelete={removeRisk} onExport={exportRisks} />}
    ...
    {page === "logs" && <LogsPage logs={data.logs} />}{page === "admin" && role === "admin" && <AdminPage profiles={profiles} records={data.records} onRoleChange={updateProfileRole} onAdd={addRecord} onUpdate={updateRecord} onDelete={removeRecord} />}
  </div></main></div>;
}
