...
const taskAction: Record<TaskRecord["status"], string> = { "待查阅": "标记已读", "已读": "提交现场反馈", "已反馈": "确认完成", "已完成": "已完成" };
const eventAction: Record<string, string> = { "待研判": "完成研判", "已研判": "启动处置", "处置中": "复核并结案", "已结案": "已结案" };
...
export function CommandPage({ currentUserId, activeEventId, events, tasks, profiles, organizations, writable, onSelectEvent, onOpenRelated, onAddEvent, onProgressEvent, onDeleteEvent, onAddTask, onProgressTask, onDeleteTask }: {
  ...
  return <>
    ...
    {selectedEvent ? <section className="incident-detail" aria-label="事件详情">
      ...
      <div className="actions"><button onClick={() => onOpenRelated("plans", selectedEvent.id)}>匹配/查看预案</button><button onClick={() => onOpenRelated("resources", selectedEvent.id)}>调度资源</button><button onClick={() => onOpenRelated("inventory", selectedEvent.id)}>申请物资</button><button onClick={() => onOpenRelated("reviews", selectedEvent.id)}>进入复盘</button><button className="primary" disabled={!writable || selectedEvent.status === "已结案" || (selectedEvent.status === "处置中" && eventTasks.some((item) => item.status !== "已完成"))} onClick={() => onProgressEvent(selectedEvent)}>{eventAction[selectedEvent.status] || "更新状态"}</button>{eventTasks.length === 0 && selectedEvent.status === "待研判" && <button className="danger" disabled={!writable} onClick={() => onDeleteEvent(selectedEvent.id)}>删除空事件</button>}</div>
